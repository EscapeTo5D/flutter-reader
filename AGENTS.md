...
- **`followSysRate=true` 已实装（2026-07-16）**：对齐原生 legado `ttsFlowSys`——系统 TTS 用系统设置 `Settings.Secure.TTS_DEFAULT_RATE`；HTTP TTS 用默认档位 1.0（后端合成无"系统设置"概念，取原生 `speechRatePlay = defaultSpeechRate`）。⚠️ 不能用 `setSpeechRate(1.0)` 偷懒——会强制覆盖用户的系统设置。实装：`AloudEngine.play/setRate` 加可选 `followSysRate` 参数（默认 false 向后兼容）；`AloudController.setFollowSysRate` 切换时若 playing 则调 `engine.setRate` 触发重排（让速率切换生效）。
- ⚠️ **跟随系统必须重建引擎，不能只"不调 setSpeechRate"（2026-07-16 修复）**：Android `TextToSpeech.setSpeechRate` 是**持久覆盖**语义——一旦调过自定义值，之后"不调 setSpeechRate"**并不会**让引擎回到系统默认，引擎沿用上次值。故上一条实装里"跳过 setSpeechRate"是不够的（bug：用户改过自定义速率再切跟随系统，速率不变）。对齐原生 legado `upSpeechRate`(ttsFlowSys=true 分支) 的 `clearTTS() + initTts()`——**销毁旧 TextToSpeech + new 新实例**，全新实例从未被 setSpeechRate 过 → 自然用系统 `TTS_DEFAULT_RATE`。flutter_tts 4.2.5 **无 dispose/shutdown API**，`new FlutterTts()` 也**无效**（Dart 对象 stateless，原生 plugin 单例不重建）；唯一能重建底层 `TextToSpeech` 的 Dart 路径是 `setEngine(engineName)`——插件内 `tts = TextToSpeech(ctx, listener, engine)` 新建实例 + init 后自动重绑 `UtteranceProgressListener`（成员，不随重建失效）+ init 期方法调用走 pending 队列不丢，对调用方透明。实装：`SystemTtsEngine` 加 `_engineRateClean` 标志跟踪引擎是否被污染，每次 setSpeechRate 后置 false；跟随系统时若已污染，调 `_ensureEngineCleanForFollowSys()`（`getDefaultEngine` + `setEngine` 重建）置回 true。重建开销 ~50-200ms 引擎 re-init，仅触发一次（后续跟随系统不再 setSpeechRate，标志保持 true）。
